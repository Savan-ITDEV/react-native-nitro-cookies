...
- `getAll()` on Android queries a list of `knownUrls` and aggregates/deduplicates cookies (workaround for Android CookieManager not exposing all-domain iteration). Returns the same shape as iOS.
