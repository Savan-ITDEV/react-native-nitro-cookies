...
const NitroCookiesHybridObject = NitroModules.createHybridObject('NitroCookies');
function cookiesToDictionary(cookies) {
  const result = {};
...
export const NitroCookies = {
   ...
   * ```
   */
  async setFromResponse(url, value) {
   ...
   *
   * - **iOS**: Returns all cookies from WKHTTPCookieStore or NSHTTPCookieStorage.
   * - **Android**: Queries a set of known app URLs via CookieManager and aggregates
   *   the results (Android does not expose a native all-domain iterator).
   *
   * @param useWebKit - (iOS only) If true, retrieve from WKHTTPCookieStore instead of NSHTTPCookieStorage
   *
   ...
   * ```
   */
  async clearByName(url, name, useWebKit) {
